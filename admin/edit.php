...
if (isset($_POST['update'])) {
    ...
    $query = "UPDATE jadwal SET kelas = '$kelas', hari = '$hari', min = '$min_val', max = '$max_val', matkul = '$matkul', dosen = '$dosen', ruang = '$ruang' WHERE id = '$id'";
    $result = mysqli_query($con, $query);
    if ($result) {
                        ...
                        <div class="col-md-12">
                            <form action="" method="post">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" value="<?php echo $row['hari']?>" name="hari">
                                </div>
                                <div class="form-group">
                                    <label for="min_val">Jam awal</label>
                                    <input type="text" class="form-control" value="<?php echo $row['min']?>" name="min" min="1" max="12">
                                </div>
                                <div class="form-group">
                                    <label for="max_val">Jumlah Jam</label>
                                    <input type="text" class="form-control" value="<?php echo $row['max']?>" name="max" min="1" max="12">
                                </div>
                                <div class="form-group">
                                    <label for="matkul">Mata Kuliah</label>
                                    <input type="text" class="form-control" value="<?php echo $row['matkul']?>" name="matkul">
                                </div>
                                <div class="form-group">
                                    <label for="dosen">Dosen</label>
                                    <input type="text" class="form-control" value="<?php echo $row['dosen']?>" name="dosen">
                                </div>
                                <div class="form-group">
                                    <label for="ruang">Ruangan</label>
                                    <input type="text" class="form-control" value="<?php echo $row['ruang']?>" name="ruang">
                                </div>
                                
                                <input type="submit" value="Update" name="update" class="btn btn-primary">
                                <a class="btn btn-secondary" href="data_jadwal.php">Kembali</a>
                            </form>
                        </div>
                    </div>
